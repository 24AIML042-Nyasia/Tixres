...
from server_auth.resolver import (  # noqa: E402
    DEFAULT_RESOLVER_API_KEY,
    DEFAULT_RESOLVER_SECRET_KEY,
)
...
def _resolver_headers(message: str = "resolver") -> dict:
    """Shared headers for the demo resolver account."""
    return _make_headers(DEFAULT_RESOLVER_API_KEY, DEFAULT_RESOLVER_SECRET_KEY, message)
...
class TestDashboardEndpoint:
    ...
    def test_dashboard_returns_expected_keys(self, client, registered_agent):
        headers = self._seed_metrics(client, registered_agent)
        resp = client.get(
            f"/api/dashboard/{registered_agent['agent_id']}", headers=headers
        )
        assert resp.status_code == 200
        body = resp.json()
        expected_keys = [
    ...
    def test_dashboard_empty_agent_returns_nulls(self, client):
        ...
        agent = r.json()
        headers = _make_headers(agent["api_key"], agent["secret_key"])
        resp = client.get(f"/api/dashboard/{agent['agent_id']}", headers=headers)
        assert resp.status_code == 200
        body = resp.json()
...
class TestTicketsEndpoint:
    def test_get_tickets_returns_structure(self, client, registered_agent):
        resp = client.get(
            ...
            headers=_make_headers(
                registered_agent["api_key"], registered_agent["secret_key"], "tickets"
            ),
        )
...
class TestFullAgentLifecycle:
    ...
    def test_full_lifecycle(self, client):
        ...
        dash_resp = client.get(
            f"/api/dashboard/{creds['agent_id']}",
            headers=_make_headers(creds["api_key"], creds["secret_key"], "dashboard"),
        )
    ...
    def test_multiple_agents_metrics_are_isolated(self, client):
        ...
        def _register_and_seed(hostname, fingerprint, cpu_val):
            ...
            return {"agent_id": creds["agent_id"], "headers": headers}
        ...
        dash_a = client.get(
            f"/api/dashboard/{agent_a['agent_id']}", headers=agent_a["headers"]
        ).json()
        dash_b = client.get(
            f"/api/dashboard/{agent_b['agent_id']}", headers=agent_b["headers"]
        ).json()
...
class TestAlertEndpoints:
    def test_list_alerts_empty(self, client):
        resp = client.get("/api/alerts", headers=_resolver_headers())
        assert resp.status_code == 200
    ...
    def test_alert_query_and_resolution(self, client):
        ...
        resp_single = client.get(
            f"/api/alerts/{alert_id}", headers=_resolver_headers()
        )
        ...
        resp_resolve = client.post(
            f"/api/alerts/{alert_id}/resolve", headers=_resolver_headers("resolve")
        )
        assert resp_resolve.status_code == 200
        assert resp_resolve.json()["message"] == "Alert resolved successfully"
        
        # 4. Confirm Alert is closed
        resp_single_closed = client.get(
            f"/api/alerts/{alert_id}", headers=_resolver_headers("resolve-check")
        )
        ...
        assert (
            client.post(
                f"/api/alerts/{alert_id}/resolve", headers=_resolver_headers("resolve2")
            ).status_code
            == 400
        )
