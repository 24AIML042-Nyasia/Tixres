...
class TicketService:
        ...
        return (
            ...
            .filter(
                and_(
                    ...
                    # We can only merge into OPEN tickets or ACKed tickets
                    Ticket.status.in_([TICKET_STATUS_OPEN, TICKET_STATUS_ACK]),
    ...
    @staticmethod
    def acknowledge_ticket(db: Session, ticket_id: int, acked_by: str) -> bool:
        """
        Acknowledge an OPEN ticket, preventing it from being auto-closed by ResolutionService
        and visually marking it as 'investigating' for other resolvers.

        Args:
            db: Active SQLAlchemy session
            ticket_id: ID of the ticket to acknowledge
            acked_by: Identifier of the user making the request

        Returns:
            True if status changed from OPEN to ACK; False if it was already ACK/CLOSED.
        ...
        ticket.acknowledged_at = datetime.now()
